...
class ProtoBufBinOneExportor: IDataActionVisitor<CodedOutputStream>
{
    public static ProtoBufBinOneExportor Ins { get; } = new();
        ...
        foreach (var field in type.Fields)
        {
            ...
            switch (field)
            {
                ...
                default:
                {
                    ...
                    field.Apply(ProtobufBinExportor.Ins, temp);
                    break;
                }
            }
        }
    ...
    private void WriteList(TType elementType, int fieldId, List<DType> datas, CodedOutputStream x)
    {
        if (elementType.Apply(IsProtobufPackedType.Ins))
        {
            ...
            {
                data.Apply(ProtobufBinExportor.Ins, temp);
            }
            temp.Flush();
            ms.Seek(0, SeekOrigin.Begin);
            x.WriteBytes(FromStream(ms));
        ...
        else
        {
            ...
            foreach (var data in datas)
            {
                ...
                data.Apply(ProtobufBinExportor.Ins, x);
            }
        }
    }
    private void WriteMap(DMap type, int fieldId, CodedOutputStream x)
    {
        ...
        foreach (var e in type.Datas)
        {
            ...
            e.Key.Apply(ProtobufBinExportor.Ins, temp);
            temp.WriteTag(2, valueType.Apply(ProtobufWireTypeVisitor.Ins));
            e.Value.Apply(ProtobufBinExportor.Ins, temp);
            temp.Flush();
            ms.Seek(0, SeekOrigin.Begin);
            x.WriteBytes(FromStream(ms));
